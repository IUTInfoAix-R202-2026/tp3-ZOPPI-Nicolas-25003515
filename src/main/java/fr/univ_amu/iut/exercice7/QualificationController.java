...
public class QualificationController {
  ...
  private void initialize() {
    ...
    tableView
        ...
        .addListener(
            (obs, ancien, nouveau) -> {
              if (nouveau == null)
                labelLecture.setText("(sélectionnez une séquence dans le tableau)");
              else
    ...
    // Bind labelVerdictGlobal au verdict du modèle : affiche "(à saisir)" si vide,
    // sinon "Verdict global : <verdict>".
    labelVerdictGlobal
        .textProperty()
        .bind(
  ...
  private void enregistrerVerdict() {
    ...
    if (choiceBoxVerdict.getValue() != null) nuit.setVerdictGlobal(choiceBoxVerdict.getValue());
  }
