...
public class FormulaireConnexionController {
    ...
    BooleanBinding motDePasseInvalide =
        new BooleanBinding() {
          ...
          protected boolean computeValue() {
            String motDePasse = champMotDePasse.textProperty().get();
  ...
  private void valider() {
    ...
    for (int i = 0; i < champMotDePasse.textProperty().get().length(); ++i) {
      masque.append('*');
    }
    labelMessage.setText(champIdentifiant.textProperty().get() + " " + masque.toString());
  }
